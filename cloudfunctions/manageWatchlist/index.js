...
exports.main = async (event) => {
  ...
  try {
    if (action === "add") {
      ...
      const exist = await col.where({ _openid: uid, fundCode }).count();
      if (exist.total > 0) return { code: 0, msg: "已加自选" };
      await col.add({ data: { _openid: uid, fundCode, fundName, group: "", createTime: new Date() } });
      return { code: 0, msg: "加自选成功" };
    }

    if (action === "remove") {
      if (!fundCode) return { code: 400, msg: "缺少基金代码" };
      await col.where({ _openid: uid, fundCode }).remove();
      return { code: 0, msg: "取消自选" };
    }

    if (action === "list") {
      const res = await col.where({ _openid: uid }).orderBy("createTime", "desc").get();
    ...
    if (action === "setGroup") {
      ...
      await col.where({ _openid: uid, fundCode: _.in(fundCodes) }).update({ data: { group: name } });
      return { code: 0, msg: "已更新分组" };
    }

    if (action === "getGroups") {
      const res = await col.where({ _openid: uid }).field({ group: true }).get();
    ...
    if (action === "renameGroup") {
      ...
      await col.where({ _openid: uid, group: oldName }).update({ data: { group: newName } });
    ...
    if (action === "deleteGroup") {
      ...
      await col.where({ _openid: uid, group: name }).update({ data: { group: "" } });
